...
This was run on the cluster since wang is too slow.

```{r}
# setwd("/home/people/leolun/projects/ontologyDescent/wangBenchmark")
# load("env.RData")
# 
# library(parallel)
# library(simplifyEnrichment)
...
#   # print(n)
#   sampleR(ontoHclust = clustNet2, n = sample(5:50,1))
# }, mc.cores = 40)
# datWang <- datWang[sapply(datWang, length)>0]
# # hist(sapply(datWang, function(x) length(unique(unlist(x)))))
# 
...
allParametersPlot
ggsave("figures/network.parameters.pdf", width = 10, height = 6.2)
```
...
```{r}
# save.image() #change this stupid name .Rdata to env.Rdata
```
...
#   temp <- lapply(all_clustering_methods()[c(-1)], function(x){
#     cluster_terms(temp,
#                   method = x,
#                   catch_error = T, 
#                   verbose = T)
#   })
#   
#   clusteringMethods <- all_clustering_methods()[c(-1,-4,-11)]
#   
...
```{r}
truth <- lapply(c("datClust", "datJia", "datJack"), function(x){
  x <- get(x)
  lapply(x, function(i){
    x <- reshape2::melt(i)
...
names(truth) <- c("TopoDist", "Jiang", "Jackard")
```
...
```{r}
clusterColors <- RColorBrewer::brewer.pal(11,"Spectral")[c(1:4,7:11)]
names(clusterColors) <- names(resOptimization$TopoDist)
...
FM index analysis suggests that that Wang method and TopoDist perform similarly on the topodist benchmark.

Wang + networks seems to generate larger and fewer clusters, at the cost of proportion of clustered terms.

TopoDist + leiden generates the best coverage with smaller cluster size than Wang. 

Final combinations: TopoDist+leiden, Wang+apcluster, remaining + walktrap

## Compare best combination of methods on the three benchmarks

```{r}
#############
#Classic distances + walktrap

resCompare <- lapply(c("Jiang", "Lin", "Rel", "Resnik"), function(dist){
  mclapply(truth, function(bench){
    lapply(bench, function(n){
...
names(resCompare) <- c("Jiang", "Lin", "Rel", "Resnik")

#############
#OntoDesc

resCompare$TopoDist <- mclapply(truth, function(bench){
  lapply(bench, function(n){
    targets <- unique(names(n))
    test <- clustereR(net, GOnames, GOlength, targets, method = "leiden")$res
    test <- structure(test$clusterNumber, names = test$ontoID)
...
resCompare$Wang <- mclapply(truth, function(bench){
  lapply(bench, function(n){
    targets <- unique(names(n))
    test <- cluster_terms(GO_similarity(targets, "MF", measure = "Wang"),
                          method = "apcluster",
                          catch_error = T,
                          verbose = T)
    test <- try(structure(as.numeric(test), names = targets))
...
beepr::beep()
# save(file = "data/resultsComparinson.Rdata", x = resCompare)
load("data/resultsComparinson.Rdata")
```

calculate the rand index for each benchmark and also run the random bench.

```{r}
#############
#Comparison

resCompareFM <- mclapply(resCompare, function(meth){
