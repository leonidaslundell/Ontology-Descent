...
If you are running this with new data, make sure that every ontology term provided is found in the network as things are curently crashing.

```{r}
targets <- example_data$ontoID[1:50]
#filter out any terms not found in the network.
targets <- target[targets %in% V(net)$name]

results <- clustereR(net, GOnames, GOlength, targets, 
